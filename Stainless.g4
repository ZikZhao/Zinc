...
if_statement
    : KW_IF OP_LPAREN condition_=expr OP_RPAREN then_=code_block (KW_ELSE else_=code_block)?
    ;
...
for_statement
    : KW_FOR OP_LPAREN (
      ...
      condition_=expr? OP_SEMICOLON
      update_=expr? OP_RPAREN
      body_=code_block
    ;
...
expr
    : <assoc = right> left_=expr op_=(OP_ASSIGN |
        OP_ADD_ASSIGN | OP_SUB_ASSIGN | OP_MUL_ASSIGN | OP_DIV_ASSIGN | OP_REM_ASSIGN |
        OP_AND_ASSIGN | OP_OR_ASSIGN | OP_BITAND_ASSIGN | OP_BITOR_ASSIGN | OP_BITXOR_ASSIGN |
        OP_LSHIFT_ASSIGN | OP_RSHIFT_ASSIGN) right_=expr # AssignExpr
    | <assoc = left> left_=expr op_=(OP_EQ | OP_NEQ) right_=expr # EqualityExpr
    | <assoc = left> left_=expr op_=(OP_LT | OP_LTE | OP_GT | OP_GTE) right_=expr # RelationalExpr
    ...
    | <assoc = left> left_=expr op_=(OP_MUL | OP_DIV | OP_REM) right_=expr # MultiplicativeExpr
    | <assoc = right> op_=(OP_INC | OP_DEC | OP_SUB | OP_NOT | OP_BITNOT) expr_=expr # UnaryExpr
    | constant_=constant # ConstExpr
    | identifier_=identifier # IdentifierExpr
    | func_=identifier OP_LPAREN (arguments_+=expr (OP_COMMA arguments_+=expr)*)? OP_RPAREN # CallExpr
